			...
			switch ($gal_layout)
			{
				...
				case 'small':
					...
					<article <?php post_class('gallery_listing  small grid_4 '.$linebreak) ?> id="post-<?php the_ID(); ?>"><a href="<?php if ($preview_url)
							...
							{
								the_permalink();
							} ?>" <?php echo ($use_lightbox) ? 'data-pp="lightbox['.get_the_ID().']"' : ''; ?>  title="<?php echo the_title(); ?>" class="imgborder thumb  <?php echo $ext; ?>"><?php get_theme_post_thumbnail(get_the_ID(), 'gallery_big'); ?></a></article>
				<?php } ?>	
				<?php	
				break;
				...
				<article <?php post_class('gallery_listing '.$layout) ?> id="post-<?php the_ID(); ?>">
						...
						<a href="<?php if ($preview_url)
							...
							{
								the_permalink();
							} ?>" <?php echo ($use_lightbox) ? 'data-pp="lightbox['.get_the_ID().']"' : ''; ?>  title="<?php echo the_title(); ?>" class="imgborder thumb  <?php echo $ext; ?>"><?php get_theme_post_thumbnail(get_the_ID(), 'gallery_big'); ?></a>	
				<?php } ?>		   
					<div class="postcontent  clearfix">   
						<h2 class="entry-title"><?php if ($icon)
				{ ?><img src="<?php echo $icon ?>" alt="<?php the_title() ?>"  ><?php } ?><a href="<?php the_permalink(); ?>" ><?php the_title(); ?></a></h2>
