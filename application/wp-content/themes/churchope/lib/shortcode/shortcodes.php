...
{
	array_push($buttons, "highlight", "notifications", "buttons", "divider", "toggle", "tabs", "contactForm", "price_table_group", 'social_link', 'social_button', 'teaser', 'testimonials');
				...
				case 'medium':
					...
					<article <?php post_class($term_id.' gallery_listing  small grid_4 '.$linebreak) ?> id="post-<?php the_ID(); ?>">
						...
						<a href="<?php if ($preview_url)
							...
							{
								the_permalink();
							} ?>" <?php echo ($use_lightbox) ? 'data-pp="lightbox['.get_the_ID().']"' : ''; ?>  title="<?php echo the_title(); ?>" class="imgborder thumb  <?php echo $ext; ?>"><?php get_theme_post_thumbnail(get_the_ID(), 'gallery_small'); ?></a>	
				<?php } ?>		   
...
function toggle_shortcode($atts, $content = null)
{
	...
	return '<div class="toggle toggle-' . $type . '"><h4 class="trigger"><span class="t_ico"></span><a href="#">' . $title . '</a></h4><div class="toggle_container">' . do_shortcode($content) . '</div></div>';
...
function th_social_button($atts, $content = null)
{
	...
	switch($button) 
	{
		...
		case 'facebook':
			$shortcode_js = <<<JS
				<script>window.fbAsyncInit=function(){FB.init({appId:'YOUR_APP_ID',channelUrl:'//WWW.YOUR_DOMAIN.COM/channel.html',status:true,cookie:true,xfbml:true});};
				(function(d){var js, id = 'facebook-jssdk', ref = d.getElementsByTagName('script')[0];if (d.getElementById(id)) {return;}js = d.createElement('script'); js.id = id; js.async = true;
				js.src = "//connect.facebook.net/en_US/all.js";ref.parentNode.insertBefore(js, ref);}(document));</script>
JS;
			$template = <<<HTML
...
function th_video($atts, $url = null)
{
	...
	if($id = youtube_id_from_url($url))
	{
		...
		$html = <<<HTML
			<div id="id$id" style="width:{$w}px;height:{$h}px"></div>
			<script type="text/javascript">
					jQuery(document).ready(function() {
