...
////////////////////////
	...
	function widget( $args, $instance ) {
		...
		$data =  $this->getTimeToSelectedEvent($instance[self::EVENT_FIELD]);
		if(is_object($data))
		{
	...
	function form( $instance ) {
		...
		<div>
			<?php if($events):?>
				...
				<?php if (isset($instance[self::EVENT_FIELD]) 
								&& $instance[self::EVENT_FIELD] == 'next')
								{
								...
								echo "<option $selected value='next'>Next Nearest</option>";
				?>
...
//						die();
	 ...
	 * @return \stdClass|0
	 */
	function getTimeToSelectedEvent($id)
	{
		if($id != 'next')
		{
		...
		{
			$result = $this->getNextNearestEvent();
			if($result)
			{
				return $result;
			}
	 ...
	 * @param string $format 
	 */
	function getNextMonthEventDate($date, $format = 'm/d/Y H:i')
	{
		if(is_integer($date))
		{
	...
	*/
	function getNextMonth($base_time = null, $months = 1)
	{
	 ...
	 * @return int 
	 */
	function getTimeBeforeEvent($event_id, $start_from = false)
	{
	 ...
	 */
	function getEventsTimeDetails($event_id)
	{
		global $wpdb;
		if($wpdb)
		{
...
SQL;
	 ...
	 * @return mix
	 */
	function getFeaturedEventsList()
	{
		global $wpdb;
		if($wpdb)
		{
...
SQL;
	 ...
	 * @return mix
	 */
	function getNotRepeatingMonthEvents($month = null, $year = null)
	{
		global $wpdb;
		if(is_null($month) && is_null($year))
		{
